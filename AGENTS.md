...
- The session lock, bash watcher, turn-end guard, no-mistakes integration, secondmates, X mode, and full PR lifecycle below describe the upstream Unix implementation and are not ported in `pwsh/` yet.
...
On macOS and Linux, run `bin/fm-session-start.sh` exactly once at session start.
...
The numbered lock, bootstrap, wake, and supervision lifecycle below applies to the upstream macOS/Linux flow.
The Windows MVP does not yet coordinate multiple primary sessions, so run only one Windows primary against a given `FM_HOME`.
